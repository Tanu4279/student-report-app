...
menu = ["🏠 Home", "➕ Add Student", "📋 All Records", "🔍 Search Student", "✏️ Modify Student", "🗑️ Delete Student", "📊 Class Result"]
choice = st.sidebar.radio("Menu", menu)

if choice == "🏠 Home":
    st.subheader("Welcome to Student Report Card System")
    st.markdown("This is a **Streamlit-based version** of the C++ Report Card project.")
    ...
    if st.button("Save Record"):
        ...
        if roll in df["RollNo"].values:
            ...
            new_data = pd.DataFrame([[roll, name, p, c, m, e, cs, per, grade]], columns=df.columns)
    ...
    if not student.empty:
        ...
        name = st.text_input("Enter New Name", value=student.iloc[0]["Name"])
        ...
        st.warning("Enter a valid Roll Number to modify.")
    ...
    if df.empty:
        ...
        st.dataframe(df[["RollNo", "Name", "Percentage", "Grade"]])
